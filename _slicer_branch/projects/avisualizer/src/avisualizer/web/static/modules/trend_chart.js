...
export function drawTrendChart(options) {
  ...
  const lineCut = lineCutFractions || { lineMin: 0, lineMax: 1 };
  const isLineCutActive = lineCut.lineMin > 0 || lineCut.lineMax < 1;
  if (isLineCutActive && hasSeriesIndices && values.length > 1) {
    const totalSamples = Number.isFinite(seriesPayload.totalSamples)
      ? seriesPayload.totalSamples
      : sampledIndices[sampledIndices.length - 1] + 1;
    const maxLineIndex = Math.max(0, totalSamples - 1);
    const targetLineIndex = lineCut.lineMax * maxLineIndex;

    let markerSeriesIndex = 0;
    for (let i = 0; i < sampledIndices.length; i += 1) {
      if (sampledIndices[i] <= targetLineIndex) {
        markerSeriesIndex = i;
      } else {
        break;
      }
    ...
    const markerX = padX + (markerSeriesIndex / (values.length - 1)) * plotW;
    const markerY = mapY(Math.max(yMin, Math.min(yMax, values[markerSeriesIndex])));

    ctx.save();
    ctx.setLineDash([4, 4]);
    ctx.beginPath();
    ctx.moveTo(markerX, headerSpace);
    ctx.lineTo(markerX, headerSpace + plotH);
    ctx.strokeStyle = "rgba(255, 187, 102, 0.95)";
    ctx.lineWidth = 1.25;
    ctx.stroke();
    ctx.restore();
