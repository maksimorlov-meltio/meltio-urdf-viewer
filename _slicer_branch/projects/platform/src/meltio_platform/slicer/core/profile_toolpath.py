...
:class:`~meltio_platform.slicer.profile.FeatureSettings` so every feature type gets its own
...
:mod:`meltio_platform.slicer.core.toolpath` so there is a single implementation of densifying,
...
def _contour_moves(
    ...
    # When perimeters print first, start the fill from where they end so the
    # shortest-travel ordering picks the nearest segment to the head.
    infill_cursor = (
        np.asarray(perimeter_moves[-1].points, dtype=float)[-1]
        if perimeter_moves and not profile.infill_before_perimeters
        else None
    )
    infill_moves = _infill_moves(
        polygon, z, layer_index, profile, infill, clearance, infill_cursor
    )
...
def _dist2(a, b) -> float:
    """Squared distance between two points (for nearest-neighbour comparisons)."""
    d = np.asarray(a, dtype=float) - np.asarray(b, dtype=float)
    return float(d.dot(d))


def _order_infill_shortest_travel(
    moves: list[ToolpathMove], cursor: np.ndarray | None
) -> list[ToolpathMove]:
    """Greedily reorder open infill segments to minimise travel.

    Repeatedly picks the unprinted segment whose nearest endpoint is closest to
    the current head position, flipping it so it starts at that endpoint. This
    fills one side of a hole before crossing it (instead of the scan-line order,
    which crosses the hole to finish the far half of each line). ``cursor`` is the
    head position the fill starts from (the previous perimeter's end, or ``None``
    to start at the first generated segment).
    """
    remaining = list(moves)
    ordered: list[ToolpathMove] = []
    cur = None if cursor is None else np.asarray(cursor, dtype=float)
    while remaining:
        if cur is None:
            best_i, flip = 0, False
        else:
            best_i, flip, best_d = 0, False, float("inf")
            for i, move in enumerate(remaining):
                pts = np.asarray(move.points, dtype=float)
                d_start = _dist2(cur, pts[0])
                d_end = _dist2(cur, pts[-1])
                dist, this_flip = (
                    (d_start, False) if d_start <= d_end else (d_end, True)
                )
                if dist < best_d:
                    best_d, best_i, flip = dist, i, this_flip
        move = remaining.pop(best_i)
        if flip:
            move.points = np.asarray(move.points, dtype=float)[::-1]
            if move.orientations is not None:
                move.orientations = np.asarray(move.orientations, dtype=float)[::-1]
        ordered.append(move)
        cur = np.asarray(move.points, dtype=float)[-1]
    return ordered
...
def _infill_moves(
    ...
    With ``profile.infill_order == "shortest_travel"`` the clipped segments are
    reordered nearest-first from ``cursor`` (the head's start position); otherwise
    they keep the scan-line (boustrophedon) order.
    """
    ...
    if profile.infill_order == "shortest_travel":
        moves = _order_infill_shortest_travel(moves, cursor)
    return moves
        ...
        for polygon in layer.polygons:
            ...
            if spacing is not None and not interior.is_empty:
                ...
                if profile.infill_order == "shortest_travel":
                    ...
                    infill_moves = _order_infill_shortest_travel(infill_moves, cursor)
