...
#### 3.3.1 The third edge — the `window` bus
Those modules reach outside themselves three ways, and only two are obvious.
ES imports are visible in the source; the `deps` object is visible in each
`initXxx(deps)` signature. The third is a handful of **globals on `window`**,
and it is invisible at every level: each read is guarded (`if
(window.MeltioErrors)`), so a missing global is a silent no-op, not an error.
That is how an embedder can wire every DOM id and every dep and still get a
console that shows no faults.

The bus is now enumerated in **`contract-dom.json` → `globals`**, generated by
`tools/gen_dom_contract.mjs` and kept honest by gate 8. Two entries matter more
than the rest:
