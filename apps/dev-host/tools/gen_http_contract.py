...
* `calledBy`, from scanning everything the release branch publishes for the
  path — the hmi/ + viewer/ partitions AND the assembly (`app.js`) with the two
  helper modules that travel with it — which separates "the consumer needs
  this" from "only the dev host's own wiring uses it". Names here are the ones
  the CONSUMER sees, so the assembly appears as `app.js`, not by its repo path.
...
_STATIC = REPO_ROOT / "apps" / "dev-host" / "src" / "avisualizer" / "web" / "static"
# The partitions are not the whole published set. tools/gen_artifact.mjs also
# ships the ASSEMBLY (urdf_viewer.js, as app.js) and the two point-cloud helpers
# it imports relatively, and the release branch carries all of them. A route
# that only the assembly calls is still a route the embedder has to provide —
# leaving them out of `calledBy` marked six real requirements as optional.
# Keyed by the name the CONSUMER sees, since that is who reads this file.
PUBLISHED_FILES = {
    "app.js": _STATIC / "urdf_viewer.js",
    "modules/api.js": _STATIC / "modules" / "api.js",
    "modules/render.js": _STATIC / "modules" / "render.js",
}
...
def _published_callers(path: str) -> list[str]:
    ...
    for published_name, source in sorted(PUBLISHED_FILES.items()):
        if source.is_file() and _mentions_path(
            source.read_text(encoding="utf-8", errors="replace"), path
        ):
            callers.append(published_name)
    return callers


def _mentions_path(text: str, path: str) -> bool:
    """Substring match, but only at a real path boundary on BOTH sides.

    Three ways a naive substring search lies, all of them observed here:

    * trailing name: `/slicer` matches the comment "see hmi/slicerPane.js" and
      the contract claims a caller that never issues the request;
    * leading segment: `/urdf` matches inside `/api/urdf/models`, which put the
      dev host's own HTML page on the list of routes an embedder must serve —
      exactly the route a consumer must NOT reimplement;
    * trailing segment: `/api/sensors` matches inside `/api/sensors/binary`, so
      a caller of the second is credited to the first.

    So: the character before must not end a name, and the character after must
    neither continue one nor open another segment. Real call sites append a
    query string or close the literal, never another segment — there are no
    path parameters in this app to make that ambiguous.
    """
    start = 0
    while True:
        ...
        starts_clean = not (before.isalnum() or before in {"_", "-"})
        ends_clean = not (after.isalnum() or after in {"_", "-", "/"})
...
def build() -> dict:
    ...
    return {
        "$comment": (
            ...
            "The HTTP surface an embedder of the published modules must "
            "provide. `calledBy` non-empty means a PUBLISHED file calls it, so an "
