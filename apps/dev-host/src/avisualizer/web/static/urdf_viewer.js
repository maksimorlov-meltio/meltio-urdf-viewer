...
import {
  createSceneCore,
  REST_RENDER_PIXEL_RATIO,
  INTERACTION_RENDER_PIXEL_RATIO,
  IDLE_RENDER_ACTIVE_WINDOW_MS,
  INTERACTION_QUALITY_HOLD_MS,
  ...
  MIN_DYNAMIC_RENDER_PIXEL_RATIO,
  DYNAMIC_QUALITY_SAMPLE_ALPHA,
  DYNAMIC_QUALITY_DOWN_FRAME_MS,
  DYNAMIC_QUALITY_UP_FRAME_MS,
  DYNAMIC_QUALITY_DOWN_STEP,
...
import {
  ...
  isMaterialsMenuOpen,
  hotspotMaterialsFocusSpoolKey,
  clearMaterialsFocusSpool,
...
// elements and listeners. Instantiated here, before anything can call into it:
...
const slicerPaneUi = createSlicerPaneUi({
  ...
  getCloudModelPopupEl: () => cloudModelPopupEl,
  getSelectedFileName: () => selectedCloudLibraryFileName || cloudStlFileSelectEl?.value || "",
  setSelectedCloudLibraryFile: (file, opts) => setSelectedCloudLibraryFile(file, opts),
  setCloudFileRowSliceStatus: (file, status) => setCloudFileRowSliceStatus(file, status),
  loadCloudOverlayFromSelectedFile: () => loadCloudOverlayFromSelectedFile(),
});
...
function isTrustedSlicerMessage(event) {
  const frameWindow = slicerPaneUi.getFrameWindow();
  return Boolean(event && frameWindow && event.source === frameWindow);
}
...
// Scene core (renderer/scene/camera/controls/lights/IBL/grid) lives in
// viewer/core/sceneCore.js — extracted in step-5 phase B. The render-quality
// tuning constants are re-exported from there for the animate loop below.
const {
  renderer, scene, camera, controls,
  topLight, viewerLight, ambientLight,
...
void ambientLight; void viewerLight; // owned by sceneCore; kept for symmetry
...
const calendarUi = createCalendarUi({
  ...
  markUserActivity: () => markUserActivity(),
  toLocalDateTimeInputValue: (dateLike) => toLocalDateTimeInputValue(dateLike),
  materialLibrary: MELTIO_MATERIAL_LIBRARY,
  getFileLibraryEntries: () => cloudFileLibraryEntries,
  onOpen: () => {
...
function collapseFilesForPrint() {
  ...
  slicerPaneUi.clearAnchoredGeometry();
}
...
async function startDockedPrint() {
  ...
  try {
    ...
    slicerPaneUi.setFullscreen(false);    // leave full view for the 3D scene
    collapseFilesForPrint();       // hide the Files list; dock the controls
    slicerPaneUi.setMenuOpen(false);      // docked bar starts as just Stop/Pause/Slicer
...
function setCloudModelMenuOpen(isOpen, options = {}) {
  ...
  if (!isCloudModelMenuOpen) {
    slicerPaneUi.setMenuOpen(false);
...
// Called every frame from animate(): the door "open" reading flips mid-glide
// (it reads the animated joint value), so it must be observed per frame — but
// the DOM is only written when the derived state changes.
let lastQuickFrontDoorIsOpen = null;
...
function updateQuickFrontDoorToggleButton() {
  ...
  if (isOpen !== lastQuickFrontDoorIsOpen) {
    lastQuickFrontDoorIsOpen = isOpen;
...
// Same per-frame observation / change-only DOM write pattern as the quick
// front-door button above.
let lastTopDoorShortcutKey = "";
...
function updateTopDoorShortcutButton() {
  ...
  const stateKey = `${hasControlData}|${isOpen}|${getLocale()}`;
  if (stateKey === lastTopDoorShortcutKey) {
    return;
  }
  lastTopDoorShortcutKey = stateKey;
...
// URDF parsing + robot-tree construction live in viewer/robot/urdfRobot.js
// (step-5 phase B2). Everything stateful stays HERE and is injected as hooks:
// mesh loading/caching, styling, the per-link material registries, and joint
// bookkeeping.
function handleRobotLinkVisual(linkName, meshObject) {
  if (linkName === "user_step_link") {
    registerUserStepMaterials(meshObject);
    applyUserStepTransparency();
  }

  if (linkName === "display_link") {
    registerDisplayMaterials(meshObject);
    applyDisplayTransparency();
  }

  if (linkName === "head_link") {
    registerHeadMaterials(meshObject);
    registerHeadVisual(meshObject);
    applyHeadTransparency();
  }

  if (linkName === WIRE_DRUM_LINK) {
    registerWireDrumMaterials(meshObject);
    applyWireDrumAppearance();
  }

  if (linkName === SPOOL_1_LINK) {
    registerSpool1Meshes(meshObject);
    applyWireDrumAppearance();
  }

  if (linkName === SPOOL_2_LINK) {
    registerSpool2Meshes(meshObject);
  }

  if (linkName === SPOOLS_DOOR_LINK) {
    registerSpoolsDoorMeshes(meshObject);
  }

  if (linkName === WIRE_SPOOL_DOOR_LINK) {
    registerWireSpoolDoorMeshes(meshObject);
  }
}

const urdfBuildHooks = {
  loadMeshObject: (filename, urdfUrl) => loadMeshObject(filename, urdfUrl),
  styleMeshTree: (object3d) => styleMeshTree(object3d),
  onLinkVisual: handleRobotLinkVisual,
  onJointState: (state, initialValue) => {
    setJointValue(state, initialValue);
...
async function loadUrdf(urdfUrl) {
  ...
  try {
    ...
    const builtRobot = await buildRobotTree(parsed, urdfUrl, urdfBuildHooks);
    if (loadToken !== activeLoadToken) {
      return;
    }
...
// Per-frame while a print runs (progress moves every frame); when idle it
// settles after one hide pass and then skips all DOM work.
let lastPrintProgressActive = null;
...
// it memoizes its full input tuple and only touches the DOM when something it
// renders actually changed — the door "open" reading flips mid-glide (it reads
// the animated joint value), so per-frame observation is required but per-frame
// DOM writes are not.
let lastBottomNavStateKey = "";

function updateBottomNavState() {
  const navSimState = printSim ? printSim.getState() : "idle";
  const stateKey = [
  ...
  lastBottomNavStateKey = stateKey;
  ...
  if (navSlicerToggleEl) {
    ...
    navSlicerToggleEl.setAttribute("aria-pressed", slicerPaneUi.isMenuOpen() ? "true" : "false");
    navSlicerToggleEl.classList.toggle("is-active", slicerPaneUi.isMenuOpen());
  }
...
window.addEventListener("pointerdown", (event) => {
  ...
  if (isMaterialsMenuOpen) {
    const isInsideMaterialsMenu = isTargetInsideMaterialsPopup(target);
...
if (navSlicerToggleEl) {
  navSlicerToggleEl.addEventListener("click", () => {
    ...
    slicerPaneUi.setMenuOpen(!slicerPaneUi.isMenuOpen());
    updateBottomNavState();
  });
}
...
updateFeederCameraAnchorButtons();
updateFeederWheelFloatingControls();
// Materials UI domain (hmi/materials.js) — owns the three materials surfaces,
// their listeners and the UI selection state; scene/hotspot/slicer edges are
// injected here. It calls initMaterialsState() itself with its own selection
// hooks, so boot only restores persistence afterwards (as before).
initMaterialsUi({
  markUserActivity: () => markUserActivity(),
  clamp,
  OVERLAY_MENU_SAFE_MARGIN_PX,
  setModelLift: (isRaised) => { materialsModelLiftTargetM = isRaised ? MATERIALS_MENU_MODEL_LIFT_M : 0; },
  setCloudModelMenuOpen: (open, opts) => setCloudModelMenuOpen(open, opts),
  isCloudModelMenuOpen: () => isCloudModelMenuOpen,
  setSlicerFullscreen: (open, opts) => slicerPaneUi.setFullscreen(open, opts),
  isSlicerFullscreen: () => slicerPaneUi.isFullscreen(),
  getSelectedCloudLibraryFileName: () => selectedCloudLibraryFileName,
  setSelectedCloudLibraryFile: (file, opts) => setSelectedCloudLibraryFile(file, opts),
  loadSlicerIframeForFile: (file) => slicerPaneUi.loadIframeForFile(file),
  startDockedPrint: () => startDockedPrint(),
  updateBottomNavState: () => updateBottomNavState(),
  updateCloudPrintSimulationControls: () => updateCloudPrintSimulationControls(),
  refreshFeedstockVisibility: () => refreshFeedstockVisibility(),
  setSpoolAssemblyHighlight: (spoolKey, opts) => setSpoolAssemblyHighlight(spoolKey, opts),
  SPOOL_HIGHLIGHT_DURATION_MS,
  getActiveSpoolHighlightKey: () => activeSpoolHighlightKey,
  setWireDrumConnected: (connected) => setWireDrumConnected(connected),
  setActiveHotspotPanel: (panelId) => setActiveHotspotPanel(panelId),
  HOTSPOT_PANEL_MATERIALS_ID,
  getActiveHotspotPanelId: () => activeHotspotPanelId,
  setKeepHotspotContextPanelVisible: (keep) => { keepHotspotContextPanelVisible = keep; },
  setToggleButtonState,
  escapeHtml,
  updateFilesSelectedSpoolFeederButtons: () => updateFilesSelectedSpoolFeederButtons(),
  feederFeedType,
  persistFeederFeedType: () => persistFeederFeedType(),
  hideNumericKeypad: () => hideNumericKeypad(),
  getNumericKeypadInput: () => numericKeypadInputEl,
});
restorePersistedMaterialsState();
populateHotspotMaterialSelect();
updateSpoolSelectionCards();
...
// Fan/Chiller utilities domain (hmi/utilities.js) — owns fan/chiller state,
// the topbar toggles/popovers/numpad and the fault records in the notification
// center; scene effects react through these hooks (event-driven, not polled).
initUtilities({
  markUserActivity: () => markUserActivity(),
  showPrintNotice: (text) => showPrintNotice(text),
  syncDustExhaustFan: () => syncDustExhaustFan(),
  setChillerVisible: (visible) => setChillerVisible(visible),
  getInertPhase: () => inertPhase,
  getNotificationsUi: () => notificationsUi,
});
...
setCloudStlStatus("idle");
setMotionStatus("idle");
// Cloud file library domain (hmi/fileLibrary.js) — owns the Files-menu file
// list, its filters/favorites/selection state and listeners; scene loads,
// THREE thumbnails and slicer flows are injected here.
initFileLibrary({
  markUserActivity: () => markUserActivity(),
  setToggleButtonState,
  updateBottomNavState: () => updateBottomNavState(),
  updateCloudPrintSimulationControls: () => updateCloudPrintSimulationControls(),
  setCloudStlStatus: (text) => setCloudStlStatus(text),
  clearCloudOverlays: () => clearCloudOverlays(),
  clearCloudStlObject: () => clearCloudStlObject(),
  loadCloudOverlayFromSelectedFile: () => loadCloudOverlayFromSelectedFile(),
  loadFileToSlicer: (name) => slicerPaneUi.loadFileToSlicer(name),
  openStartPrintPreview: (name) => openStartPrintPreview(name),
