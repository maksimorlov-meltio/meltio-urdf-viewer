...
} from "../../src/avisualizer/web/static/sim/toolpathModel.js";
...
test("buildLineSegmentBuffers counts segments, layers and path length", () => {
  ...
  assert.equal(buffers.monotonicLayers, true);
});
...
test("buildLineSegmentBuffers flags out-of-order layers", () => {
  const buffers = buildLineSegmentBuffers(
    {
      moves: [
        { points: [0, 0, 1, 10, 0, 1], kind: "infill", layer: 1 },
        { points: [0, 0, 0, 10, 0, 0], kind: "infill", layer: 0 },
      ],
    },
    { unitScale: 1 },
  );
  assert.equal(buffers.monotonicLayers, false);
});
