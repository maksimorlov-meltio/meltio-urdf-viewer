// Assembly annotation manager (extracted byte-exact from urdf_viewer.js).
// Draws SVG callouts anchored to robot parts, hides occluded ones, and on click
// runs the corresponding door/cover/spool action + camera focus. Deeply coupled
// to the god-file: reads core mutable state (camera/controls/robotRoot) via ctx
// getters and calls ~20 god-file action/helper functions passed through ctx by
// name. createAssemblyAnnotationManager(ctx) -> { clear, rebuildFromRobot,
// update, onResize }; the god-file drives update() from the RAF loop.
...
export function createAssemblyAnnotationManager(ctx) {
  const {
    layerEl,
    getCamera,
    getControls,
    ...
    getKeepHotspotContextPanelVisible,
  ...
  const collectOcclusionMeshes = () => {
    ...
    if (!getRobotRoot() || !ENABLE_ANNOTATION_OCCLUSION) {
      return;
    }

    getRobotRoot().traverse((node) => {
  ...
  const rebuildRobotLocalCorners = () => {
    ...
    const localBounds = computeObjectLocalBounds(getRobotRoot());
  ...
  const updateModelScreenBounds = () => {
    ...
    if (!getRobotRoot() || !robotLocalCorners.length) {
      return;
    }
    ...
    for (const localCorner of robotLocalCorners) {
      ...
      getRobotRoot().localToWorld(silhouetteCornerWorld);
      silhouetteCornerProjected.copy(silhouetteCornerWorld).project(getCamera());
  ...
  const computeOccluded = (item, worldPoint, isProjectedOutside) => {
    ...
    rayDirection.copy(worldPoint).sub(getCamera().position);
    ...
    raycaster.set(getCamera().position, rayDirection);
    raycaster.near = 0.01;
  ...
  const getItemIsOpen = (itemId) => {
    ...
    if (itemId === "spools-door") {
      if (isFrontDoorOpen()) {
        return getActiveHotspotPanelId() === HOTSPOT_PANEL_MATERIALS_ID;
      }
  ...
  const resolveTargetObject = (definition) => {
    ...
    const primaryTarget = definition.targetObjectName
      ? getRobotRoot().getObjectByName(definition.targetObjectName)
      : null;
    if (primaryTarget) {
    ...
    return getRobotRoot().getObjectByName(definition.fallbackTargetObjectName);
  };
  ...
  const hasControlData = (itemId) => {
    ...
    if (itemId === "feeder-drive") {
      return Boolean(getLeftFeederWheelState() || getRightFeederWheelState());
    }
    if (itemId === "top-cover") {
  ...
  const rebuildFromRobot = () => {
    ...
    getRobotRoot().updateWorldMatrix(true, true);
  ...
  const update = (nowMs = performance.now()) => {
    ...
    cameraForward.copy(getControls().target).sub(getCamera().position).normalize();
    ...
    if (ENABLE_ANNOTATION_OCCLUSION) {
      cameraMoved = (
        !hasCameraSnapshot ||
        previousCameraPosition.distanceToSquared(getCamera().position) > 1e-8 ||
        previousControlsTarget.distanceToSquared(getControls().target) > 1e-8 ||
        (1 - Math.abs(previousCameraQuaternion.dot(getCamera().quaternion))) > 1e-7
      );

      previousCameraPosition.copy(getCamera().position);
      previousControlsTarget.copy(getControls().target);
      previousCameraQuaternion.copy(getCamera().quaternion);
    ...
    const shouldUseFilesPopupRail = getIsCloudModelMenuOpen();
    setHotspotTriggerRailVisible(shouldUseFilesPopupRail);

    if (!shouldUseFilesPopupRail && getActiveHotspotPanelId() && !getKeepHotspotContextPanelVisible()) {
      closeHotspotContextPanel();
    }
    ...
    for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
      ...
      projected.copy(anchorWorld).project(getCamera());
      ...
      // Keep callouts clear of fixed top and bottom menus while getCamera() moves.
      ...
      centerToAnchor.copy(anchorWorld).sub(getControls().target);
      let sideScore = 1;
      let facingDot = -1;
